...
router.put('/education', [ auth, [
    ...
    try {
        ...
        const profile = await Profile.findOne({ user: req.user.id }); // req.user.id which we get from the jsonwebtoken/auth middleware
        profile.education.unshift(newEdu); // object that we created above
        await profile.save(); 
        res.json(profile);
...
router.delete('/education/:edu_id', auth, async (req, res) => {
    try {
        ...
        // foundProfile.education = foundProfile.education.filter((exp) => exp._id.toString() !== req.params.edu_id);
        // OR Get remove index
